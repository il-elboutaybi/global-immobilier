...
exports.OPTIONS = {
    ...
    transform({ name, options, values: [value] }) {
        ...
        if (tlsInsecure) {
            // Allow insecure connections only in non-production environments
            if (process.env.NODE_ENV !== 'production') {
                process.env.NODE_TLS_REJECT_UNAUTHORIZED = "0";
                options.rejectUnauthorized = false;
                options.checkServerIdentity = !options.tlsAllowInvalidHostnames ? undefined : () => undefined;
            } else {
                throw new Error("Insecure TLS connections are not allowed in production.");
            }
        } else {
            // Ensure secure connections in all environments
            process.env.NODE_TLS_REJECT_UNAUTHORIZED = "1";
            options.rejectUnauthorized = true;

            // Set other options based on configuration
            ...
            // Adjust checkServerIdentity based on tlsAllowInvalidHostnames
            options.checkServerIdentity = options.tlsAllowInvalidHostnames ? () => undefined : undefined;
        }
        return tlsInsecure;
    }
